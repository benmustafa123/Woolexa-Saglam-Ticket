...
client.on("interactionCreate", async interaction => {
    ...
    return Math.floor(Math.random() * (max - min + 1)) + min;
        ...
        const channel = await interaction.guild.channels.create({
            name: `│${getRandomInt(100, 8000)}│${interaction.user.username}`,
          ...
          .setTitle("**Başarıyla Ticket Oluşturdun**")
          .setDescription('```ansi\n[2;31m[2;31mAşağıdan destek almak istediğin konuyu seç[0m[2;31m[0m```\n```ansi\n[2;34mLütfen destek oluşturduğunda yetkili arkadaşlarımızı etiketleme. Ticketın açıldığında onlara zaten bildirim gidiyor![0m```')
          .setTimestamp()
          ...
          channel.send({content:`<@${interaction.user.id}>`,embeds: [tickecreatembed], components: [ticketselect] })
